...
tr = lambda string: QApplication.translate("chessboard", string)
...
class Board(QWidget):
    ...
    def field_state(self, field) -> None:
        ...
        if self.chess.place_queen(field):
            ...
            QMessageBox.critical(self, tr("Forbidden"), tr("Cannot place Queen here!"))
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        #size and title bar
        self.setWindowTitle(tr("N-Queens solver"))
    ...
    def ui_setup(self) -> None:
        ...
        self.actions = {name: QAction(tr(name).capitalize(), self) for name in names}
        ...
        self.multi = QCheckBox(tr("Multiprocess solving"), self)
        ...
        self.statuslabel = QLabel(tr("{} known solutions").format(self.solutions.count()))
        self.statusbar.addPermanentWidget(self.statuslabel)

    def new_chessboard(self) -> None:
        """ Ask user for dimension, clear the solution list and create new chessboard. """
        dim, ok = QInputDialog.getInt(self, tr("New Chessboard"), tr("Provide new chessboard's dimension:"))
        if ok :
    ...
    def solve_chessboard(self) -> None:
        ...
        self.statusbar.showMessage(tr("Computation in progress"))
    ...
    def finish_computation(self) -> None:
        ...
        self.statusbar.showMessage(tr("Finished"), 4000)
        ...
        self.statuslabel.setText(tr("{} known solutions").format(self.solutions.count()))
        QMessageBox.information(self, tr("Solved"), tr("Found {} solutions in {} seconds.").format(self.solutions.count(), self.solver.time))
        self.solver.time = '?'
